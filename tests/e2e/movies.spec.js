...
test("Não deve cadastrar quando o filme já estiver cadastrado", async ({
    page,
    request,
}) => {
    ...
    await page.popup
        .haveText(`O título '${movie.title}' já consta em nosso catálogo. Por favor, verifique se há necessidade de atualizações ou correções para este item.
    `);
});
...
test("Deve realizar busca pelo termo zumbi", async ({ page, request }) => {
    ...
    await page.waitForResponse((response) => {
        return response.url().includes("/movies") && response.status() === 200;
    });
});
