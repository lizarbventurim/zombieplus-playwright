...
test("Não deve cadastrar um lead na fila de espera quando o email já tiver cadastrado", async ({
  page,
}) => {
  await page.leads.visit();
  await page.leads.openLeadModal();
...
test("Não deve cadastrar quando nenhuma informação é preenchida", async ({
  ...
  await page.leads.alertHaveText([requiredFiledMessage, requiredFiledMessage]);
});
